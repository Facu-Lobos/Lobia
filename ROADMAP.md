...
- [x] **Recepción de pacientes**: sala de espera / cola visual y estado del turno en tiempo real (ya existe `arrivedAt` en secretaría). Implementado como vista "Sala de espera" (secretaría, encargado, admin y profesional) con 4 estados: pendiente → esperando → en consulta → atendido, refrescada automáticamente cada 15s. Navegable por día (← anterior / siguiente, o fecha puntual) y filtrable por profesional (staff); usa la misma tabla `Appointment` que "Mis Turnos" del paciente, así que un turno reservado online aparece ahí directo, sin duplicar datos.
