...
- **Infraestructura de base de datos (18/09/2026)**: `DATABASE_URL` pasó del pooler de Supabase en modo **Session** (puerto 5432) al modo **Transaction** (puerto 6543, mismo host). Session mode reserva una conexión física fija por cada cliente conectado y con sólo 15 disponibles en el plan actual, cualquier combinación de tráfico real + dev local + scripts sueltos lo agotaba (`EMAXCONNSESSION`, se veía como "Error occurred in the Server Components render" genérico en el navegador). Transaction mode multiplexa muchos clientes sobre pocas conexiones reales — pensado para apps serverless como esta. **Este cambio hay que replicarlo a mano en las variables de entorno de Vercel** (no viaja con el `git push`, `.env` está gitignoreado) y volver a desplegar.
...
- [x] **Llamador**: pantalla de texto grande pensada para un TV en la sala de espera física (`/secretaria|encargado|admin|profesional/llamador`, 18/09/2026: se agregó la versión del profesional, misma pantalla compartida por todos los roles de staff), que muestra el paciente que está pasando a "en consulta" junto con el `consultingRoom` (consultorio) configurado en el profesional. Se actualiza sola cada 8s. `Professional.consultingRoom` es editable desde "Datos generales" en la ficha del profesional (admin/encargado) **o por el propio profesional** al entrar: el login de un especialista pasa primero por `/profesional/consultorio` ("¿En qué consultorio vas a atender?", gateado en `proxy.ts` vía cookie `consultorio_ok`, se re-pregunta en cada login) antes de dejarlo entrar a cualquier otra pantalla.
  - Fix (18/09/2026): el llamador filtraba por la **fecha agendada del turno** en vez de por **cuándo se llamó** — si el profesional avanzaba el estado de un turno agendado para otro día (por ej. probando la función en un día sin turnos hoy), `calledAt` se seteaba bien pero no aparecía en el llamador porque `date` no caía "hoy". Ahora filtra por `calledAt` directamente, sin mirar `date`.
  - **Versión pública** (18/09/2026, sin login): pensada para dejar abierta en el TV/monitor físico de la sala de espera sin que nadie tenga que iniciar sesión en ese dispositivo. `/llamador` muestra las de todas las sedes juntas; `/llamador/<slug>` (ej. `/llamador/clinica-del-sol`, `/llamador/sede-norte`) filtra a una institución puntual — el slug se arma al vuelo desde `Institution.name` (`lib/slug.ts`, sin persistir nada), así que alcanza con crear la institución para que su URL funcione sola. Layout a pantalla completa (`components/PublicLlamadorScreen.tsx`, compartido entre ambas rutas): header con el logo real + "Tecnología inteligente para tu salud", columna de turnos a la izquierda (con texto grande — si llaman a dos pacientes seguido tiene que alcanzar a leerse antes de que se corra de la lista) y la tarjeta grande con el paciente actual a la derecha, con el texto "{Nombre y apellido} — Acérquese al consultorio {número} con Dr/Dra {nombre del profesional}" (se le saca cualquier "Dr."/"Dra." que ya tuviera cargado el nombre, `lib/format.ts`, para no duplicarlo). Tapa el header/nav del sitio con un overlay (no requirió tocar el layout raíz) y lleva `noindex` para que no la indexe un buscador con nombres de pacientes.
    - Sonido al llamar (18/09/2026): cuando cambia el paciente de la tarjeta principal (`components/LlamadorChime.tsx`) se reproduce un sonido de notificación (`public/sounds/llamador-chime.mp3`, provisto por el cliente) — no suena en la carga inicial de la página, sólo en cada cambio posterior. Mismo componente sumado al llamador de staff (`CallerScreen`) y al público. Los navegadores bloquean el audio hasta el primer click/touch/tecla en la página; en un TV que se deja abierto sin tocar, el primer llamado puede no sonar hasta esa primera interacción (se "destraba" reproduciendo y pausando el audio en silencio en ese primer toque) — después queda desbloqueado el resto de la sesión.
...
- [x] **HCE (Historia Clínica Electrónica)** — versión inicial: `PatientAntecedents` (alergias, enfermedades crónicas, medicación habitual, compartido entre todos los profesionales que atendieron al paciente) y `ClinicalNote`, rediseñado el 18/09/2026: dejó de ser "una nota estructurada (motivo/diagnóstico/notas/tratamiento) por turno, editable" para ser una **bitácora de texto libre** — cada "Guardar" agrega una entrada nueva (no pisa la anterior) y el campo queda vacío para seguir escribiendo, con link u documento adjunto opcional (`ClinicalNote.linkUrl`/`documentId`, este último referencia un `PatientDocument`). El especialista la carga desde `/profesional/turnos/[id]/historia-clinica` (con el historial previo del paciente, incluso de otros profesionales/turnos), a la que ahora también se llega haciendo click en el nombre del paciente desde la grilla de `/profesional/sala-espera`; el paciente ve todo (ya no hay campo oculto) en `/mis-turnos/historia-clinica`. También se puede subir ahí mismo un documento suelto (foto, análisis, informe) al `PatientDocument` del paciente, sin ligarlo a una nota — mismo mecanismo que ya usaban secretaría/encargado. Pendiente para una v2: plantillas por especialidad, permisos más finos que "cualquier especialista que lo atendió alguna vez".
