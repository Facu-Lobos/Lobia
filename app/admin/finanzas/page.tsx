...
export default async function AdminFinanzasPage({
  ...
  const [{ rows, income, expense, balance }, institutions] = await Promise.all([
    getFinanceSummary(null, monthStart, monthEnd),
    ...
    <div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex gap-2 text-sm">
          <Link
            href={`/admin/finanzas?month=${monthParam(prevMonth)}`}
            className="rounded-md border border-border px-2 py-1 hover:border-primary"
          >
            ← Anterior
          </Link>
          <Link
            href={`/admin/finanzas?month=${monthParam(nextMonth)}`}
            className="rounded-md border border-border px-2 py-1 hover:border-primary"
          >
            Siguiente →
          </Link>
      ...
      <section className="mt-8">
        ...
        <form
          ...
          className="mt-3 flex max-w-3xl flex-wrap items-end gap-2"
        >
          <div>
          ...
          <div>
            ...
            <select
              ...
              className="mt-1 rounded-md border border-border bg-surface px-3 py-2 outline-none focus:border-primary"
            >
              <option value="">General (sin institución)</option>
              {institutions.map((i) => (
            ...
            <div key={t.id} className="flex items-center justify-between py-3 text-sm">
              <div>
                ...
                <p className="text-muted">
                  {formatDate(t.date)}
                  {t.institution ? ` · ${t.institution.name}` : " · General"}
                </p>
              </div>
